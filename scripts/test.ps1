...
$checks = @(
    ...
    @{ Path = '/admin.html'; Contains = 'Administracao' },
...
if (-not $saved.saved -or $progress.completed -notcontains 'basica') { throw 'Falha na persistencia de progresso.' }
...
if ($user.user.email -ne $email -or $me.user.email -ne $email) { throw 'Falha no cadastro ou na sessao.' }
Write-Host 'OK  Cadastro e sessao'

$personalData = Invoke-RestMethod -Uri "$baseUrl/api/privacy/export" -WebSession $webSession -Headers $headers
if ($personalData.account.email -ne $email -or $personalData.consents.purpose -notcontains 'terms') { throw 'Falha na exportacao de dados pessoais.' }
...
if (-not $consent.saved -or $consent.granted) { throw 'Falha na revogacao de consentimento.' }
Write-Host 'OK  Exportacao e consentimento'
...
if ($catalog.courses.Count -ne 6 -or $lesson.lesson.courseId -ne 'basica') { throw 'Falha no catalogo ou na aula.' }
Write-Host 'OK  Catalogo e aula'
...
if ($null -ne $afterDeletion.user) { throw 'Falha na eliminacao da conta.' }
Write-Host 'OK  Eliminacao da conta'
