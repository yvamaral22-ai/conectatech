...
async function syncInitialConsents(user) {
  ...
  const rows = [
    {user_id:user.id, purpose:'terms', version:user.user_metadata.terms_version || privacyVersion, granted:true, updated_at:now},
    {user_id:user.id, purpose:'analytics', version:user.user_metadata.terms_version || privacyVersion, granted:Boolean(user.user_metadata.analytics_consent), updated_at:now},
  ];
  const {error} = await supabase.from('consents').upsert(rows, {onConflict:'user_id,purpose,version'});
...
function progressStorageKey() { return `conectatech-progress-${currentUser?.id || 'guest'}`; }
function readProgressState() {
  try { return JSON.parse(localStorage.getItem(progressStorageKey()) || '{"completed":[]}'); }
...
async function api(path, options = {}) {
  const response = await fetch(`/api${path}`, {
    ...options,
    headers: {'X-Client-Id': clientId, 'X-ConectaTech-Request': '1', 'Content-Type': 'application/json', ...(options.headers || {})}
  });
  if (!response.ok) { const body = await response.json().catch(() => ({})); throw new Error(body.error || `API indisponível (${response.status})`); }
  return response.status === 204 ? null : response.json();
}

function renderCourses(filter = 'todas') {
  const visible = courses.filter(course => filter === 'todas' || course.level === filter);
  if (!visible.length) { grid.innerHTML = '<p class="empty-state">Nenhuma trilha publicada está disponível agora.</p>'; return; }
...
function saveState() { localStorage.setItem(progressStorageKey(), JSON.stringify(state)); }
function updateProgress() {
  const percent = courses.length ? Math.round((state.completed.filter(id => courses.some(course => course.id === id)).length / courses.length) * 100) : 0;
  ...
  document.querySelector('#hero-progress-value').textContent = currentUser ? `${percent}% concluído` : (percent ? `${percent}% neste aparelho` : 'Entre para sincronizar');
...
async function loadPlatformMetrics() {
  ...
  document.querySelector('#registered-stat').textContent = Number(metrics.registered_people).toLocaleString('pt-BR');
  document.querySelector('#active-stat').textContent = Number(metrics.active_learners).toLocaleString('pt-BR');
  document.querySelector('#courses-stat').textContent = Number(metrics.available_courses).toLocaleString('pt-BR');
  document.querySelector('#completion-rate-stat').textContent = `${Number(metrics.completion_rate).toLocaleString('pt-BR')}%`;
}
...
function loadCachedCatalog() {
  try { courses = JSON.parse(localStorage.getItem(catalogCacheKey) || '[]'); } catch (_) { courses = []; }
  renderCourses(); updateProgress();
}

function escapeHtml(value) { return String(value).replace(/[&<>"']/g, character => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[character])); }
function safeExternalUrl(value) { try { const url = new URL(value); return url.protocol === 'https:' ? url.href : '#'; } catch (_) { return '#'; } }
...
document.querySelector('#dialog form').addEventListener('submit', async event => {
  ...
  try {
    ...
    const {data, error} = await supabase.from('feedback').insert({user_id:user.id, category, message}).select('id').single();
    if (error) throw error;
    document.querySelector('#offline-status').textContent = `Mensagem registrada. Protocolo ${data.id.slice(0, 8)}.`;
    document.querySelector('#offline-status').style.display = 'block'; setTimeout(() => document.querySelector('#offline-status').style.removeProperty('display'), 5000);
  } catch (_) { localStorage.setItem(`conectatech-feedback-${Date.now()}`, JSON.stringify({message, category})); }
});
...
accountButton.addEventListener('click', async () => {
  if (accountButton.dataset.authenticated === 'true') {
    document.querySelector('#perfil').scrollIntoView({behavior:'smooth'}); return;
  }
...
document.querySelector('#auth-form').addEventListener('submit', async event => {
  ...
  data.termsAccepted = data.termsAccepted === 'on'; data.analyticsConsent = data.analyticsConsent === 'on';
  try {
    let user;
    if (supabase) {
      const operation = registerMode
        ? supabase.auth.signUp({email:data.email, password:data.password, options:{data:{name:data.name, terms_version:'2026-08-28', terms_accepted:true, analytics_consent:data.analyticsConsent}}})
      ...
      if (!result.session && registerMode) {
        ...
        document.querySelector('#offline-status').textContent = 'Confira seu e-mail para confirmar a conta e depois entre.';
    ...
    currentUser = user;
    accountButton.textContent = (user.user_metadata?.name || user.name || user.email).split(' ')[0]; accountButton.dataset.authenticated = 'true'; authDialog.close();
    await loadUserProgress(); await loadMyProfile();
...
async function renderProfile(profile, targetPrefix = 'my') {
  ...
  if (avatar) {
    ...
    avatar.innerHTML = url ? `<img src="${url}" alt="">` : escapeHtml(profile.display_name.slice(0, 2).toUpperCase());
  }
  if (targetPrefix === 'my') {
    ...
    document.querySelector('#profile-bio').textContent = profile.bio || 'Nenhuma apresentação adicionada.';
    document.querySelector('#profile-city').textContent = profile.city || '';
    document.querySelector('#profile-visibility').textContent = profile.is_public ? 'Perfil público' : 'Perfil privado';
  }
}

async function loadMyProfile() {
  if (!currentUser || !supabase) return;
  const {data, error} = await supabase.from('profiles').select('id,username,display_name,bio,city,avatar_path,is_public').eq('id', currentUser.id).single();
  if (error) return;
  currentProfile = data; await renderProfile(data);
}

document.querySelector('#edit-profile').addEventListener('click', async () => {
  if (!currentUser) { setAuthMode(false); authDialog.showModal(); return; }
  if (!currentProfile) await loadMyProfile();
  if (!currentProfile) return;
  document.querySelector('#profile-display-name').value = currentProfile.display_name;
  document.querySelector('#profile-username-edit').value = currentProfile.username;
  document.querySelector('#profile-bio-edit').value = currentProfile.bio;
  document.querySelector('#profile-city-edit').value = currentProfile.city;
  document.querySelector('#profile-public-edit').checked = currentProfile.is_public;
...
document.querySelector('#profile-form').addEventListener('submit', async event => {
  ...
  try {
    ...
    const update = {display_name:String(form.get('display_name')).trim(), username:String(form.get('username')).trim().toLowerCase(), bio:String(form.get('bio')).trim(), city:String(form.get('city')).trim(), is_public:form.get('is_public') === 'on', updated_at:new Date().toISOString()};
    const file = document.querySelector('#profile-avatar-file').files[0];
    if (file) {
      if (file.size > 2097152 || !['image/jpeg','image/png','image/webp'].includes(file.type)) throw new Error('Use uma imagem JPG, PNG ou WebP de até 2 MB.');
    ...
    const {data, error} = await supabase.from('profiles').update(update).eq('id', currentUser.id).select('id,username,display_name,bio,city,avatar_path,is_public').single();
    if (error) throw error;
    await supabase.auth.updateUser({data:{name:update.display_name}});
    currentProfile = data; await renderProfile(data); document.querySelector('#profile-dialog').close();
  } catch (error) { message.textContent = error.code === '23505' ? 'Esse nome de usuário já está em uso.' : error.message; }
});

document.querySelector('#profile-search-form').addEventListener('submit', async event => {
  event.preventDefault();
  const result = document.querySelector('#profile-search-result');
  const username = document.querySelector('#profile-search').value.trim().toLowerCase();
  const {data, error} = await supabase.from('profiles').select('username,display_name,bio,city,avatar_path').eq('username', username).eq('is_public', true).maybeSingle();
  if (error || !data) { result.innerHTML = '<p class="empty-state">Perfil público não encontrado.</p>'; return; }
...
document.querySelector('#logout-button').addEventListener('click', async () => {
  ...
  await supabase.auth.signOut(); currentUser = null; currentProfile = null; state = readProgressState();
  accountButton.dataset.authenticated = 'false'; accountButton.textContent = 'Entrar'; updateProgress(); renderCourses(document.querySelector('.filter.active').dataset.filter);
  document.querySelector('#profile-name').textContent = 'Seu perfil'; document.querySelector('#profile-username').textContent = ''; document.querySelector('#profile-bio').textContent = 'Seu perfil fica vinculado com segurança à sua conta.'; document.querySelector('#profile-visibility').textContent = 'Entre para criar seu perfil';
});
...
async function exportPersonalData() {
  ...
  const [progress, consents, feedback] = await Promise.all([
    supabase.from('course_progress').select('course_id, completed_at, updated_at'),
    supabase.from('consents').select('purpose, version, granted, created_at, updated_at'),
    supabase.from('feedback').select('category, message, created_at'),
  ]);
  const error = progress.error || consents.error || feedback.error;
  if (error) throw error;
  return {account:{id:user.id, email:user.email, name:user.user_metadata?.name}, progress:progress.data, consents:consents.data, feedback:feedback.data};
}
document.querySelector('#privacy-button').addEventListener('click', async () => {
  if (accountButton.dataset.authenticated !== 'true') { setAuthMode(false); authDialog.showModal(); return; }
  const exported = await exportPersonalData().catch(error => { document.querySelector('#offline-status').textContent = error.message; return null; });
  if (!exported) return;
  const analytics = exported.consents.find(item => item.purpose === 'analytics');
  document.querySelector('#analytics-consent').checked = Boolean(analytics?.granted);
  document.querySelector('#privacy-message').textContent = '';
  privacyDialog.showModal();
});
document.querySelector('#analytics-consent').addEventListener('change', async event => {
  try {
    if (supabase) {
      const user = await supabaseUser();
      const {error} = await supabase.from('consents').upsert({user_id:user.id, purpose:'analytics', version:privacyVersion, granted:event.target.checked, updated_at:new Date().toISOString()}, {onConflict:'user_id,purpose,version'});
      if (error) throw error;
    } else await api('/privacy/consent', {method:'POST', body:JSON.stringify({purpose:'analytics', granted:event.target.checked})});
    document.querySelector('#privacy-message').textContent = 'Preferência de consentimento salva.';
  }
  catch (error) { event.target.checked = !event.target.checked; document.querySelector('#privacy-message').textContent = error.message; }
});
document.querySelector('#export-data').addEventListener('click', async () => {
  try { const data = await exportPersonalData(); const blob = new Blob([JSON.stringify(data, null, 2)], {type:'application/json'}); const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = 'conectatech-meus-dados.json'; link.click(); URL.revokeObjectURL(link.href); }
  catch (error) { document.querySelector('#privacy-message').textContent = error.message; }
});
document.querySelector('#delete-account').addEventListener('click', async () => {
  ...
  try { await api('/privacy/delete', {method:'POST', body:JSON.stringify({password, confirmation})}); localStorage.removeItem('conectatech-state'); accountButton.dataset.authenticated = 'false'; accountButton.textContent = 'Entrar'; privacyDialog.close(); location.reload(); }
  catch (error) { document.querySelector('#privacy-message').textContent = error.message; }
});
...
async function loadUserProgress() {
  try {
    if (supabase) {
      ...
      const {data, error} = await supabase.from('course_progress').select('course_id');
      if (error) throw error;
      state = {completed:data.map(item => item.course_id)};
    } else state = {completed:(await api('/progress')).completed};
    saveState(); updateProgress(); renderCourses(document.querySelector('.filter.active').dataset.filter);
  } catch (_) { state = readProgressState(); updateProgress(); renderCourses(document.querySelector('.filter.active').dataset.filter); }
}
async function saveRemoteProgress(courseId) {
  try {
    if (supabase) {
      if (!currentUser) return;
      const now = new Date().toISOString();
      const {error} = await supabase.from('course_progress').upsert({user_id:currentUser.id, course_id:courseId, completed_at:now, updated_at:now}, {onConflict:'user_id,course_id'});
      if (error) throw error;
    } else await api('/progress', {method:'POST', body:JSON.stringify({courseId})});
  } catch (_) { /* O progresso local será preservado até a próxima sincronização. */ }
}
async function initializeFromServer() {
  try { await loadCatalog(); }
  catch (_) { loadCachedCatalog(); }
  try {
    const user = supabase ? (await supabase.auth.getUser()).data.user : (await api('/me')).user;
    if (user) { currentUser = user; await syncInitialConsents(user); accountButton.textContent = (user.user_metadata?.name || user.name || user.email).split(' ')[0]; accountButton.dataset.authenticated = 'true'; await loadUserProgress(); await loadMyProfile(); }
  } catch (_) { /* O uso sem conta permanece disponível. */ }
  updateProgress(); loadPlatformMetrics(); loadOpportunities();
}
...
if (depthScene && matchMedia('(pointer:fine)').matches && !matchMedia('(prefers-reduced-motion:reduce)').matches) {
  depthScene.addEventListener('pointermove', event => {
    const bounds = depthScene.getBoundingClientRect();
    depthScene.style.setProperty('--rotate-y', `${((event.clientX - bounds.left) / bounds.width - .5) * 8}deg`);
    depthScene.style.setProperty('--rotate-x', `${((event.clientY - bounds.top) / bounds.height - .5) * -8}deg`);
  });
  depthScene.addEventListener('pointerleave', () => { depthScene.style.setProperty('--rotate-x', '0deg'); depthScene.style.setProperty('--rotate-y', '0deg'); });
}
