...
function render() {
  ...
  grid.innerHTML = projects
    .map(
      (project) =>
        `<article class="project-card"><div class="project-card-top"><span>Projeto</span><div><button type="button" data-edit="${project.id}">Editar</button><button type="button" data-delete="${project.id}">Excluir</button></div></div><h2>${escapeHtml(project.title)}</h2><p>${escapeHtml(project.description)}</p>${project.project_url ? `<a class="text-link" href="${safeExternalUrl(project.project_url)}" target="_blank" rel="noopener">Abrir projeto →</a>` : ""}</article>`,
    )
    .join("");
}
async function load() {
...
form.addEventListener("submit", async (event) => {
  ...
  const currentForm = event.currentTarget,
    message = document.querySelector("#project-message"),
    values = Object.fromEntries(new FormData(currentForm));
  const id = values.id;
  delete values.id;
  const operation = id
    ? supabase
